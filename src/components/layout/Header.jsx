...
function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#050507]/70 backdrop-blur-xl transition-all duration-300 dark:border-white/10 dark:bg-[#050507]/70 light:border-black/10 light:bg-white/70">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
        <a href="#home" className="group inline-flex items-center gap-3 text-lg font-semibold tracking-tight text-white transition-colors duration-300 dark:text-white light:text-black">
        ...
        <nav className="hidden items-center gap-6 text-sm text-white/70 transition-colors duration-300 dark:text-white/70 light:text-black/70 md:flex lg:gap-8">
          {navItems.map((item) => (
            <a
              key={item.label}
              href={item.href}
              className="group relative transition-colors duration-300 hover:text-white dark:hover:text-white light:hover:text-black"
            >
              {item.label}
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            onClick={toggleTheme}
            className="inline-flex items-center justify-center rounded-full p-2 transition-colors duration-300 hover:bg-white/10 dark:hover:bg-white/10 light:hover:bg-black/10"
